...
final class Admin {
	...
	public function render() {
		...
		$total    = count( $rules );
		$active   = count( array_filter( $rules, static function ( $rule ) { return ! empty( $rule['enabled'] ); } ) );
		$paused   = $total - $active;
				...
				<section class="vtx-card vtx-main-card">
					...
					<div class="vtx-panel" id="vtx-panel-logs" role="tabpanel" aria-labelledby="vtx-tab-logs" data-vtx-panel="logs" hidden>
						...
						<div class="vtx-log-list">
							<?php if ( empty( $logs ) ) : ?><p><?php esc_html_e( 'No activity yet.', 'vtx-redirects' ); ?></p><?php endif; ?>
		...
		if ( 'add' === $action ) {
			$rule = Normalizer::rule(
				array(
					...
					'code'        => isset( $_POST['code'] ) ? wp_unslash( $_POST['code'] ) : 301,
			...
			$bulk   = isset( $_POST['bulk_rules'] ) ? wp_unslash( $_POST['bulk_rules'] ) : '';
	...
	public function ajax_usage() {
		...
		$this->logger->add( __( 'Usage scan', 'vtx-redirects' ), sprintf( __( 'Found %d matching content items for %s.', 'vtx-redirects' ), count( $results ), Normalizer::source_path( $source ) ) );
	 ...
	 * @return void
	 */
	private function code_select( $name, $selected, $class = '' ) {
		$labels = array(
